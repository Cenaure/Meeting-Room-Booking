...
1. **Час зберігається в UTC** на стороні сервера. Сервер завжди очікує отримати час у форматі UTC та виконує
   ...
   Сервер також перетворює час на київський під час створення повторюваних бронювань,
   щоб запобігти помилці, повʼязаній зі зсувом бронювань на годину раніше чи пізніше
   під час переходу на літній/зимовий час. Завдяки цьому всі бронювання серії
   залишаються в один і той самий день тижня та в той самий час за місцевим годинником,
   навіть якщо серія перетинає межу переведення годинників.
...
Скопіюйте приклади конфігурації, прибравши суфікс `.example`:

```bash
cp .env.example .env && \
cp nest-server/.env.development.local.example nest-server/.env.development.local && \
cp nest-server/.env.test.local.example nest-server/.env.test.local && \
cp next-web/.env.development.local.example next-web/.env.development.local
```
...
Щойно сервер та вебзастосунок повністю запустяться, відкрийте браузер і перейдіть за адресою:
...
Тести можна запустити командами в директорії сервера
...
| 7 | **Інтеграційні тести API**               | Інтеграційні тести тестують створення, скасування бронюваннь, та відмови валідації                                                                                       |
| 8 | **Адаптивний інтерфейс**                 | Повна підтримка десктопів, планшетів і мобільних пристроїв.                                                                                                              |
