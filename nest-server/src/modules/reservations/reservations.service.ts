...
export class ReservationsService {
  constructor(
    ...
    @Inject(RESERVATIONS_QUEUE_EVENTS)
    private readonly queueEvents: QueueEvents,
    @InjectQueue('reservations-queue')
  ...
  validateReservationTime(timeStart: Date, timeEnd: Date, room: Room) {
    ...
    const [roomStartHour, roomStartMinute] = room.working_hours_start
      .split(':')
      .map(Number);
    const [roomEndHour, roomEndMinute] = room.working_hours_end
      .split(':')
      .map(Number);

    const workDayStart = start.set({
      hour: roomStartHour,
      minute: roomStartMinute,
      second: 0,
      millisecond: 0,
    });
    const workDayEnd = start.set({
      hour: roomEndHour,
      minute: roomEndMinute,
      second: 0,
      millisecond: 0,
    });
  ...
  async createReservation(userId: number, dto: CreateReservationDto) {
    ...
    const job = await this.reservationsQueue.add(
      ReservationType.SingleReservation,
      {
        ...dto,
        user,
      },
  ...
  async createReservationSeries(
    ...
    const job = await this.reservationsQueue.add(
      ReservationType.ReservationSeries,
      {
        ...dto,
        user,
      },
  ...
  async findAdjacentReservations(query: GetReservationsDto) {
    ...
    const [leftAdjacent, rightAdjacent] =
      await this.databaseService.$transaction([
        this.databaseService.reservation.findFirst({
          where: { room_id, time_end: start_date },
        }),
        this.databaseService.reservation.findFirst({
          where: { room_id, time_start: end_date },
        }),
      ]);
  ...
  async getUserReservations(userId: number, query: GetMyReservationsDto) {
    ...
    const filterOptions: ReservationWhereInput = {
      ...
      ...(query.filter === ReservationFilter.PAST && {
        time_end: { lte: now },
      }),
    };
  ...
  async cancelReservation(userId: number, reservationId: string) {
    // default - 15 minutes
    const preventCancellationBeforeMinutes = this.configService.get(
      'reservation.prevent_cancellation_before_minutes',
    );
    ...
    if (
      DateTime.fromJSDate(reservation.time_start, { zone: 'UTC' }).diff(
        ...
        'minutes',
      ).minutes < preventCancellationBeforeMinutes
    )
      throw AppException.badRequest({
    ...
    if (leftAdjacent)
      await this.notificationSchedulerService.cancelScheduleReservationEndingNotification(
        leftAdjacent.id,
      );
  ...
  async cancelReservationSeries(userId: number, reservationSeriesId: string) {
    ...
    const reservations =
      await this.findReservationsBySeriesId(reservationSeriesId);
    ...
    if (reservations[0].reserved_by !== userId) throw AppException.forbidden();

    const now = DateTime.now().setZone('UTC');
    const hasTooLateReservation = reservations.some(
      (reservation) =>
        DateTime.fromJSDate(reservation.time_start).diff(now, 'minutes')
          .minutes < preventCancellationBeforeMinutes,
    );
    ...
    await Promise.all(
      updatedReservations.map(async (updatedReservation) => {
        const { leftAdjacent, rightAdjacent } =
          await this.findAdjacentReservations({
            room_id: updatedReservation.room_id,
