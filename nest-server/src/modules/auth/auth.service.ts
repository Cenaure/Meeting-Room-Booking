...
export class AuthService {
  ...
  async signIn(dto: SignInDto) {
    ...
    const password_hash =
      user?.password_hash ||
      '$2b$12$sMoGxG1IosKwXaDUV2HJvun1DzJJ2CtAZeiVY09slqq8lZfFHrGAq.placeholder';
  ...
  async generateActivationLink(userId: number) {
    ...
    if (user.is_activated)
      throw AppException.badRequest({
        code: AppExceptionBodyCode.activationNotNeeded,
        message: 'Activation not needed',
      });
  ...
  async updatePassword(userId: number, dto: UpdatePasswordDto) {
    ...
    if (!user.password_hash)
      throw AppException.conflict({
        code: AppExceptionBodyCode.noPassword,
