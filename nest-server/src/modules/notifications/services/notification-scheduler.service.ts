...
export class NotificationSchedulerService {
  ...
  async scheduleReservationEndingNotification(
    ...
    const notifyBeforeMinutes = this.configService.get("reservation.notify_before_minutes")

    const notifyDate = DateTime.fromJSDate(nextReservation.time_start).minus({minutes: notifyBeforeMinutes});
    ...
    this.logger.log(`Scheduling reservation ending soon notification for reservation ${userReservation.id} at ${DateTime.now().plus({milliseconds: delay})}`)
  ...
  async cancelScheduleReservationEndingNotification(reservationId: string) {
    const job = await this.notificationsQueue.getJob(`notification-remind-reservation-ending-${reservationId}`)
