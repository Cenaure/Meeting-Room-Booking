...
export class NotificationsService {
  ...
  async createNotification(dto: CreateNotificationDto) {
    ...
    if (!user)
      throw AppException.notFound({code: AppExceptionBodyCode.userNotFound, message: "User not found"})
  ...
  async findNotificationById(notificationId: string) {
    return this.databaseService.notification.findFirst({where: {id: notificationId}})
  }

  async findMyUnreadNotifications(userId: number) {
    return this.databaseService.notification.findMany({where: {user_id: userId, read_at: null}})
  }
}
